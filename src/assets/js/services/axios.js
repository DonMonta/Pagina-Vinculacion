...
API.interceptors.request.use(
  (config) => {
    ...
    const tokenType = localStorage.getItem("token_type_vinc");
  ...
  (error) => {
    if (error.response) {
      ...
      if (error.response.status === 401) {
        ...
        window.location.href = "/site/site-login"; // Redirige al login si la sesión caduca
      }
