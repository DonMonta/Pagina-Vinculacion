      ...
      <div>
        ...
        <form class="flex-grow">
          <div class="relative">
            ...
            <input type="text" placeholder="Ingresa el nombre del DOMINIO a buscar..." v-model="searchQuery"
      ...
      <table class="min-w-full">
        <thead>
          <tr class="border-t border-gray-100 dark:border-gray-800">
            ...
            <th class="py-5 px-4 text-left">
              <p class="font-semibold text-gray-500 text-sm dark:text-gray-400">DOMINIO</p>
            </th>
        ...
        <tbody>
          ...
          <tr v-else v-for="post in filteredarray" :key="post.id_dom_huma"
            ...
            <td class="py-5 px-4 whitespace-nowrap">
              <p class="text-gray-600 text-sm font-medium dark:text-gray-400">{{ post.id_dom_huma }}</p>
            </td>

            <td class="py-5 px-4 whitespace-nowrap">
              <div>
                <p class="font-bold text-gray-800 text-base dark:text-white/90">
                  {{ post.detalle_dom_huma }}
                </p>
              </div>
            </td>
            ...
            <td class="py-3 text-right whitespace-nowrap">
              <div class="flex justify-end gap-2">
                ...
                <button @click="eliminar(post.id_dom_huma, post.detalle_dom_huma)"
        ...
        <div
          ...
          <div class="px-2 pr-14">
            <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Agregar Dominio Académico
            </h4>
            <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Llene todos los campos para agregar un nuevo Dominio Académico.
            </p>
          </div>
            ...
            <div class="px-2 overflow-y-auto custom-scrollbar">
              <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                <div>
                  ...
                  <input type="text" v-model="objetoguardar.detalle_dom_huma"
                    placeholder="Ej: Científicos"
    ...
    <Modal v-if="isEditModalOpen" @close="isEditModalOpen = false">
      <template #body>
        <div
          ...
          <div class="px-2 pr-14">
            <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Editar Dominio Académico
            </h4>
            <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Los datos mostrados son los actuales del Dominio Académico. Realice los cambios necesarios y guarde.
            </p>
          </div>
          <form class="flex flex-col">
            <div class="px-2 overflow-y-auto custom-scrollbar">
              <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                <div>
                  ...
                  <input type="text" v-model="objetoeditar.detalle_dom_huma"
                    placeholder="Ej: Científicos"
                    class="dark:bg-dark-900 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:text-white/90" />
                  <p class="mt-1.5 text-xs text-gray-500">Use un nombre descriptivo</p>
                </div>
...
export default {
  ...
  computed: {
    ...
    formIsValid() {
      ...
      return (
        this.objetoguardar.detalle_dom_huma.trim() !== '' 
      );
    },
    formIsValidEdit() {
      return (
        this.objetoeditar.detalle_dom_huma.trim() !== ''
      );
    },
  ...
  methods: {
    abrirModalEdicion(user) {
      ...
      this.objetoeditar = {
        id_dom_huma: user.id_dom_huma,
        detalle_dom_huma: user.detalle_dom_huma,
      };
    ...
    async GetData(page = 1, searchQuery = "") {
      ...
      try {
        ...
        const response = await API.get(`${this.baseUrl}/invi_dom_huma`, { params });
    ...
    async registrar() {
      ...
      try {
        ...
        const exito = await enviarsolig('POST', params, `${this.baseUrl}/invi_dom_huma`, 'DOMINIO ACADEMICO registrado con éxito');
        if (exito) {
          ...
          mostraralertas2("No se pudo registrar el DOMINIO ACADEMICO", "error");
        }
      } catch (error) {
    ...
    async Update() {
      ...
      try {
        ...
        const exito = await enviarsolig('PUT', params, `${this.baseUrl}/invi_dom_huma/${this.objetoeditar.id_dom_huma}`, 'DOMINIO ACADEMICO actualizado con éxito');
        if (exito) {
          ...
          this.$.setupState.isEditModalOpen = false;
          mostraralertas2("No se pudo editar el DOMINIO ACADEMICO", "error");
        }
      } catch (error) {
    ...
    eliminar(id, nombre) {
      try {
        eliminacion(
          `${this.baseUrl}/invi_dom_huma/`,
          id,
          'Deshabilitar registro',
          '¿Realmente desea eliminar el DOMINIO ACADEMICO  ' + nombre + '?',
          this.actualizar   // 👈 callback para refrescar la tabla al confirmar
        );
      } catch (error) {
        console.error("Error al eliminar el DOMINIO ACADEMICO:", error);
        this.cargando = false;
      }
    },
