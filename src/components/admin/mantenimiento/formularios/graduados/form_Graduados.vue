<template>
  <div
    ...
    <div v-if="mostrarModalInscritos"
    ...
    <div v-if="mostrarModalRespuestas"
      class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 z-99999">
      <div
        class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border border-gray-100 dark:border-gray-800">
        <div
          class="p-5 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50/30 dark:bg-gray-800/30">
        ...
        <div class="p-6 overflow-y-auto flex-grow" v-if="detalleAlumno">
          <div
            class="flex flex-col sm:flex-row gap-5 p-4 bg-gray-50 dark:bg-white/[0.02] rounded-xl border border-gray-100 dark:border-gray-800 mb-6 items-center">
            ...
            <div class="text-center sm:text-left flex-grow">
              <h4 class="text-base font-bold text-gray-900 dark:text-white">{{ detalleAlumno.persona.NombInfPer }} {{
                detalleAlumno.persona.ApellInfPer }} {{ detalleAlumno.persona.ApellMatInfPer }}</h4>
              <p class="text-xs text-gray-500 mt-0.5">Cédula: {{ detalleAlumno.persona.CIInfPer }} | {{
                detalleAlumno.persona.mailInst }}</p>
              <div
          ...
          <div class="space-y-4">
            <div v-for="(item, index) in detalleAlumno.respuestas" :key="index"
              class="p-4 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-transparent">
              ...
              <div
                class="mt-3 text-sm px-3 py-2 rounded-lg flex items-center gap-2 border bg-gray-50 dark:bg-white/[0.01] border-dashed border-gray-200 dark:border-gray-800">
  ...
  methods: {
    ...
    async GetData(page = 1, searchQuery = "") {
      ...
      try {
        ...
        const response = await API.get(`${this.baseUrl}/getformGraduados`, { params });
