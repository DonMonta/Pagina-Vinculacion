    ...
    <div class="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <form class="flex-grow">
          <div class="relative">
            ...
            <input type="text" placeholder="Ingresa el tipo de proyecto de vinculación a buscar..." v-model="searchQuery"
      ...
      <div class="flex items-center gap-3">
        ...
        <button @click="isProfileAddressModal = true"
          ...
          Añadir Tipo de Proyecto de Vinculación
        </button>
      </div>
    </div>
      ...
      <table class="min-w-full">
        <thead>
          <tr class="border-t border-gray-100 dark:border-gray-800">
            ...
            <th class="py-5 px-4 text-left">
              <p class="font-semibold text-gray-500 text-sm dark:text-gray-400">Tipo de Proyecto de Vinculación</p>
            </th>
        ...
        <tbody>
          ...
          <tr v-else v-for="post in filteredarray" :key="post.id_tip_invi_proy"
            ...
            <td class="py-5 px-4 whitespace-nowrap">
              <p class="text-gray-600 text-sm font-medium dark:text-gray-400">{{ post.id_tip_invi_proy }}</p>
            </td>

            <td class="py-5 px-4 whitespace-nowrap">
              <div>
                <p class="font-bold text-gray-800 text-base dark:text-white/90">
                  {{ post.detalle_invi_proyect }}
                </p>
              </div>
            </td>
        ...
        <div
          ...
          <div class="px-2 pr-14">
            <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Agregar Tipo de Proyecto de Vinculación
            </h4>
            <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Llene todos los campos para agregar un nuevo Tipo de Proyecto de Vinculación.
            </p>
          </div>
          <form class="flex flex-col">
              ...
              <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                <div>
                  <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                    Nombre del Tipo de Proyecto de Vinculación
                  </label>
                  <input type="text" v-model="objetoguardar.detalle_invi_proyect"
                    placeholder="Ej: Proyectos especializados"
            ...
            <div class="flex items-center gap-3 mt-6 lg:justify-end">
              ...
              <button v-if="formIsValid" @click="registrar" type="button"
                ...
                Guardar Tipo de Proyecto de Vinculación
              </button>
            </div>
          </form>
        </div>
    ...
    <Modal v-if="isEditModalOpen" @close="isEditModalOpen = false">
      <template #body>
        <div
          ...
          <div class="px-2 pr-14">
            <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Editar Tipo de Proyecto de Vinculación
            </h4>
            <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Los datos mostrados son los que llenó anteriormente. Realice los cambios necesarios y guarde.
            </p>
          </div>
          <form class="flex flex-col">
            <div class="px-2 overflow-y-auto custom-scrollbar">
              <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                <div>
                  ...
                  <input type="text" v-model="objetoeditar.detalle_invi_proyect"
                    placeholder="Ej: Proyectos especializados"
                    class="dark:bg-dark-900 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:text-white/90" />
                  <p class="mt-1.5 text-xs text-gray-500">Use un nombre descriptivo</p>
                </div>
...
export default {
  ...
  computed: {
    ...
    formIsValid() {
      ...
      return (
        this.objetoguardar.detalle_invi_proyect.trim() !== '' 
      );
    },
    formIsValidEdit() {
      return (
        this.objetoeditar.detalle_invi_proyect.trim() !== ''
      );
    },
  ...
  methods: {
    abrirModalEdicion(user) {
      ...
      this.objetoeditar = {
        id_tip_invi_proy: user.id_tip_invi_proy,
        detalle_invi_proyect: user.detalle_invi_proyect,
      };
    ...
    async GetData(page = 1, searchQuery = "") {
      ...
      try {
        ...
        const response = await API.get(`${this.baseUrl}/invi_tipo_proyecto`, { params });
    ...
    async registrar() {
      ...
      try {
        const params = {
          detalle_invi_proyect: this.objetoguardar.detalle_invi_proyect
        };
        const exito = await enviarsolig('POST', params, `${this.baseUrl}/invi_tipo_proyecto`, 'Registrado con éxito');
        if (exito) {
          ...
          mostraralertas2("No se pudo registrar", "error");
        }
      } catch (error) {
    ...
    async Update() {
      try {
        const params = {
          detalle_invi_proyect: this.objetoeditar.detalle_invi_proyect
        };
        const exito = await enviarsolig('PUT', params, `${this.baseUrl}/invi_tipo_proyecto/${this.objetoeditar.id_tip_invi_proy}`, 'Actualizado con éxito');
        if (exito) {
          ...
          this.$.setupState.isEditModalOpen = false;
          mostraralertas2("No se pudo editar", "error");
        }
      } catch (error) {
        console.error("❌ Error al registrar:", error.response?.data || error);
      }
